...
fig, axs = plt.subplots(
    2, 2, figsize=set_figsize(columnwidth, .8, subplots=(2, 2)), layout="constrained",
...
axs[0,0].text(0.22, 1.2, 'temperature', fontsize=8, transform=axs[0,0].transAxes)
axs[0,1].text(0.2, 1.2, 'precipitation', fontsize=8, transform=axs[0,1].transAxes)
...
pcm = axs[0,0].contourf(anom_temp.lon, anom_temp.lat, model_temp_winter_acc.statistic, cmap=cmap, vmin=vmin, vmax=vmax, levels=levels)
axs[0,1].contourf(anom_prec.lon, anom_prec.lat, model_prec_winter_acc.statistic, cmap=cmap, vmin=vmin, vmax=vmax, levels=levels)

h = axs[0,0].contourf(anom_temp.lon, anom_temp.lat, model_temp_winter_acc.pvalue > ptresh, hatches=['+++', None], colors='none', vmin=vmin, vmax=vmax, levels=[1-ptresh,1])
h._hatch_color = (0,0,0,1)
h = axs[0,1].contourf(anom_prec.lon, anom_prec.lat, model_prec_winter_acc.pvalue > ptresh, hatches=['+++', None], colors='none', vmin=vmin, vmax=vmax, levels=[1-ptresh,1])
h._hatch_color = (0,0,0,1)

axs[0,0].contour(anom_temp.lon, anom_temp.lat, model_temp_winter_acc.statistic, ls='--', colors='k', linewidths=.5, levels=[.5])
axs[0,1].contour(anom_prec.lon, anom_prec.lat, model_prec_winter_acc.statistic, ls='--', colors='k', linewidths=.5, levels=[.5])
...
pcm = axs[1,0].contourf(anom_temp.lon, anom_temp.lat, model_temp_winter_ce, cmap=cmap, vmin=vmin, vmax=vmax, levels=levels)
axs[1,1].contourf(anom_prec.lon, anom_prec.lat, model_prec_winter_ce, cmap=cmap, vmin=vmin, vmax=vmax, levels=levels, extend='min')

axs[1,0].contour(anom_temp.lon, anom_temp.lat, model_temp_winter_ce, ls='--', colors='k', linewidths=.5, levels=[.3])
axs[1,1].contour(anom_prec.lon, anom_prec.lat, model_prec_winter_ce, ls='--', colors='k', linewidths=.5, levels=[.3])
...
plt.savefig('plots/temp_prec_skills_0wr.pdf')
